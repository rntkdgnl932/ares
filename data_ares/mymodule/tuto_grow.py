...
def tuto_grow_explain(cla):
    ...
    try:
        ...
        if v_.now_chabter == "chap_1_7":
            ...
            full_path = "c:\\my_games\\ares\\data_ares\\imgs\\tuto\\explain\\chap_1_7_inventory_title\\inventory_title.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set_(10, 10, 120, 80, cla, img, 0.7)
            if imgs_ is not None and imgs_ != False:
                ...
                for i in range(10):
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        if imgs_ is not None and imgs_ != False:
                            ...
                            full_path = "c:\\my_games\\ares\\data_ares\\imgs\\tuto\\explain\\chap_1_7_talgut\\talgut_click.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(0, 110, 960, 1050, cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                click_pos_reg(imgs_.x - 120, imgs_.y - 70, cla)
                            else:
                                full_path = "c:\\my_games\\ares\\data_ares\\imgs\\tuto\\explain\\chap_1_7_talgut\\talgut_click2.PNG"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set_(0, 110, 600, 1050, cla, img, 0.7)
                                if imgs_ is not None and imgs_ != False:
...
def burst_mode(cla):
    ...
    try:
        ...
        print(e)
        return 0
def burst_mode_2(cla):
    import pyautogui
    import numpy as np
    import cv2
    from function import imgs_set_, click_pos_2, mouse_move_cpp, drag_pos_Press, drag_pos_Release
