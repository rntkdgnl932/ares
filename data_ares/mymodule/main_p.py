...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\ares\\data_ares;./data_ares" --add-data="C:\\my_games\\ares\\mysettings;./mysettings" --name ares -i="ares_macro.ico" --add-data="ares_macro.ico;./" --icon="ares_macro.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
...
class ThirdTab(QWidget):
    ...
    def initUI(self):
        ...
        dir_path2 = dir_path + "\\ares\\mysettings\\game_server"
    ...
    def button_line_save(self):
        ...
        dir_path = "C:\\my_games"
        file_path = dir_path + "\\line\\line.txt"
        file_path2 = dir_path + "\\ares\\mysettings\\game_server\\game_server.txt"
...
class SecondTab(QWidget):
    ...
    def win_left_ex(self):
        print("왼쪽으로 정렬 합니다.")
        full_path = "c:\\my_games\\ares\\data_ares\\imgs\\check\\ares_title.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set_(960, 0, 1920, 1080, "three", img, 0.8)
        time.sleep(0.3)
        if imgs_ is not None and imgs_ != False:
            print("왼쪽 ares 보여", imgs_)
            time.sleep(0.5)
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        file_path5 = dir_path + "\\data_ares\\jadong\\jadong_force_list.txt"
        ...
        self.perfect_pause.clicked.connect(self.ares_stop_perfect)
        # self.temporary_pause = QPushButton('아레스켜기')
        # self.temporary_pause.clicked.connect(self.ares_start)
        ...
        dir_path = "C:\\my_games\\ares\\mysettings\\my_level"
    ...
    def ares_start(self):
        ...
        try:
            ...
            print("game_Playing(self): ares_start")
            # 제노니아가 원조라서 제노니아를 참고해야함...
            # # 애플리케이션 실행을 subprocess 모듈을 통해 관리자 권한으로 실행
            # subprocess.Popen([r"C:\Program Files\Hive\ares\ares.exe"])
            #
            # # 애플리케이션에 연결
            # app = Application(backend="win32").connect(title_re="ares  ")
            # # app = Application(backend="win32").start(r"C:\Program Files\Hive\ares\ares\Binaries\Win64\ares-Win64-Shipping.exe")  # 프로그램 실행
    ...
    def onActivated_slelect_spot(self, e):
        if e != 0 and e != '사냥터':
            ...
            dir_path = "C:\\my_games\\ares"
            dir_spot = "C:\\my_games\\ares\\mysettings\\gold_force"
    ...
    def onActivated_slelect_gold_read(self):
        dir_path = "C:\\my_games\\ares"
        dir_gold = "C:\\my_games\\ares\\mysettings\\gold_force"
    ...
    def onActivated_slelect_collection_toggle_read(self):
        ...
        dir_path = "C:\\my_games\\ares"
        dir_toggle = "C:\\my_games\\ares\\mysettings\\collection"
    ...
    def onActivated_one_character_level(self, text):
        ...
        if result_number_check == True:
            ...
            dir_path = "C:\\my_games\\ares"
            file_path = dir_path + "\\ares\\my_level\\one_character.txt"
            with open(file_path, "w", encoding='utf-8-sig') as file:
    ...
    def onActivated_hunt(self, text):
        global onHunt
        if text != 0 and text != '< 엘리아 >':
            onHunt = text
            print('onHunt', onHunt)
        else:
            onHunt = 'none'
            pyautogui.alert(button='넵', text='사냥터를 선택해 주시지예', title='엘리아')
            print("자동 사냥터를 선택해 주세요.")
    def onActivated_hunt2(self, text):
        global onHunt2
        if text != 0 and text != '< 루나 >':
            onHunt2 = text
            print('onHunt2', onHunt2)
        else:
            onHunt2 = 'none'
            pyautogui.alert(button='넵', text='사냥터를 선택해 주시지예', title='루나')
            print("자동 사냥터를 선택해 주세요.")
