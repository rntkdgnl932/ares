                ...
                for z in range(len(read_event)):
                    ...
                    if is_drag[1] != "pass":
                        ...
                        if drag_num == 0:
                            ...
                            if len(read_event) == 11:
                                now_y = 330
                            elif len(read_event) == 12:
                                now_y = 300
                            elif len(read_event) == 13:
                                now_y = 265
                            elif len(read_event) == 14:
                                now_y = 230
                            elif len(read_event) == 15:
                                now_y = 200
                            elif len(read_event) == 16:
                                now_y = 170
    ...
    try:
        ...
        if how == "four":
            ...
            for i in range(15):
                ...
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                # imgs_ = imgs_set_(270, 650, 310, 680, cla, img, 0.7)
                imgs_ = imgs_set_(reg_x - 20, reg_y - 20, reg_x + 20, reg_y + 20, cla, img, 0.7)
                if imgs_ is not None and imgs_ != False:
                    # 여기 받기 부분이 how에 따라 x,y 값 달라짐.
                    ...
                    for c in range(5):
                        ...
                        if imgs_ is not None and imgs_ != False:
                            ...
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(660, 660, 780, 700, cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                click_pos_reg(imgs_.x, imgs_.y, cla)
                                time.sleep(1)
                ...
                for i in range(5):
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        for z in range(5):
                            ...
                            imgs_ = imgs_set_(10, 10, 120, 100, cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                full_path = "c:\\my_games\\ares\\data_ares\\imgs\\get_items\\post_delete_btn.PNG"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set_(830, 990, 900, 1040, cla, img, 0.7)
                                if imgs_ is not None and imgs_ != False:
                                    click_pos_reg(imgs_.x, imgs_.y, cla)
